...
import { AddTagModalComponent } from 'src/app/shared/add-tag-modal/add-tag-modal.component';
import { DocumentViewerDataService } from '../data-access/document-viewer-data.service';
import { WORDS } from '../data-access/document.viewer.constants';
import { Highlight } from './document-viewer.interface';
...
export class DocumentViewerComponent {
  ...
  constructor(
    ...
    private documentViewerDataService: DocumentViewerDataService
  ) {}
  ...
  public testPagesLoaded(pagesNumber: number) {
    ...
    const pixelsHighlights = this.documentViewerDataService.getPixelsHighlights(
      WORDS,
      page.width,
      page.height
    );

    const pdfPointsHighlights =
      this.documentViewerDataService.getPDFPointsHighlights(
        pixelsHighlights,
        viewport,
        pageRect
      );
  ...
  private onSelectText(event: MouseEvent): void {
    ...
    if (content) {
      ...
      const selectedCoordinates =
        this.documentViewerDataService.getSelectedCoordinates(
          page,
          selection,
          content
        );
